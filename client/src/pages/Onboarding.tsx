...
import { Brain, ArrowRight, ArrowLeft, Loader2, CheckCircle2, Sparkles, Rocket, Pencil, Save, X } from "lucide-react";
...
const INDUSTRY_OPTIONS = ["E-Commerce", "Coaching/Beratung", "Agentur", "Lokales Business", "SaaS/Software", "Content Creator"];
const GOAL_OPTIONS = ["Mehr Leads", "Bekanntheit steigern", "Produkt-Launch", "Kundenbindung", "Social Media Wachstum"];
    ...
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/30 flex items-center justify-center p-4">
      <Card className="w-full max-w-3xl">
        ...
        <CardContent className="space-y-6">
          {/* Step 1: Welcome */}
          {step === 1 && (
            <div className={`text-center space-y-6 py-8 transition-all duration-300 ${isTransitioning ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0 animate-in fade-in slide-in-from-right-4'}`}>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: "150ms" }}>
                ...
                <CardDescription className="text-base md:text-lg max-w-xl mx-auto">
                  Dein persönlicher KI Marketing-Coach. Beantworte ein paar Fragen und ich erstelle dir eine maßgeschneiderte Roadmap.
                </CardDescription>
              </div>
              <div className="flex justify-center animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: "300ms" }}>
                <Button size="lg" variant="gradient" onClick={handleNext} className="text-lg px-8 py-6">
                  Los geht's
          ...
          {/* Step 2: Questions */}
          {step === 2 && (
            <div className={`space-y-6 transition-all duration-300 ${isTransitioning ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0 animate-in fade-in slide-in-from-right-4'}`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2 mt-1">
                    {INDUSTRY_OPTIONS.map((opt) => (
                      <Badge 
                        key={opt} 
                        variant={formData.industry === opt ? "default" : "outline"}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2 mt-1">
                    {GOAL_OPTIONS.map((opt) => (
                      <Badge 
                        key={opt} 
                        variant={formData.mainGoals.includes(opt) ? "default" : "outline"}
                        className="cursor-pointer hover:bg-primary/90 transition-colors"
                        onClick={() => {
                           ...
                           if (current.includes(opt)) {
                             // Remove
                             updateFormData("mainGoals", current.replace(opt, "").replace(", ,", ",").replace(/^, /, "").replace(/, $/, ""));
                           } else {
                             // Add
                             updateFormData("mainGoals", current ? `${current}, ${opt}` : opt);
                           }
                        }}
                      >
                        {opt}
          ...
          {step === 3 && (
            <div className={`space-y-6 transition-all duration-300 ${isTransitioning ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0 animate-in fade-in slide-in-from-right-4'}`}>
              ...
              <Card className="bg-muted/50 border-[#FF6B9D]/20 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: "150ms" }}>
            ...
            <div className={`space-y-6 transition-all duration-300 ${isTransitioning ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0 animate-in fade-in slide-in-from-right-4'}`}>
              <div className="animate-in fade-in duration-300">
                ...
                <div className="mt-3 p-3 rounded-lg bg-muted/30 border border-border/50 text-xs text-muted-foreground">
                  <strong className="text-foreground">Was ist SMART?</strong> Ein bewährtes Framework für Zielsetzung: 
                    ...
                    <CardHeader className="relative">
                      ...
                      {editingGoalIndex === index ? (
                        <div className="space-y-3 pr-12">
                          <div>
                            <Label htmlFor={`goal-title-${index}`} className="text-xs text-muted-foreground">Titel</Label>
                            <Input 
                              id={`goal-title-${index}`}
                              value={goal.title} 
                          ...
                          <div>
                            <Label htmlFor={`goal-desc-${index}`} className="text-xs text-muted-foreground">Beschreibung</Label>
                            <Textarea 
                              id={`goal-desc-${index}`}
                        ...
                        <>
                          <CardTitle className="text-lg flex items-center gap-2 pr-10">
                            <Badge className="bg-gradient-to-r from-[#FF6B9D] to-[#C44FE2] text-white border-0 shrink-0">{index + 1}</Badge>
                            {goal.title}
                          </CardTitle>
                          <CardDescription className="pr-10">{goal.description}</CardDescription>
                        </>
                      )}
                    </CardHeader>
                      ...
                      {editingGoalIndex === index ? (
                        <div className="grid gap-3 md:grid-cols-2">
                          <div className="space-y-1">
                            <Label htmlFor={`goal-s-${index}`} className="text-xs">Spezifisch</Label>
                            <Input id={`goal-s-${index}`} value={goal.specific} onChange={(e) => updateGoal(index, 'specific', e.target.value)} />
                          </div>
                          <div className="space-y-1">
                            <Label htmlFor={`goal-m-${index}`} className="text-xs">Messbar</Label>
                            <Input id={`goal-m-${index}`} value={goal.measurable} onChange={(e) => updateGoal(index, 'measurable', e.target.value)} />
                          </div>
                          <div className="space-y-1">
                            <Label htmlFor={`goal-a-${index}`} className="text-xs">Attraktiv</Label>
                            <Input id={`goal-a-${index}`} value={goal.achievable} onChange={(e) => updateGoal(index, 'achievable', e.target.value)} />
                          </div>
                          <div className="space-y-1">
                            <Label htmlFor={`goal-r-${index}`} className="text-xs">Relevant</Label>
                            <Input id={`goal-r-${index}`} value={goal.relevant} onChange={(e) => updateGoal(index, 'relevant', e.target.value)} />
                          </div>
                          <div className="space-y-1 md:col-span-2">
                            <Label htmlFor={`goal-t-${index}`} className="text-xs">Terminiert</Label>
                            <Input id={`goal-t-${index}`} value={goal.timeBound} onChange={(e) => updateGoal(index, 'timeBound', e.target.value)} />
                          </div>
                        </div>
                      ) : (
                        <>
          ...
          {/* Step 5: Complete */}
          {step === 5 && (
            <div className={`text-center space-y-8 py-8 transition-all duration-300 ${isTransitioning ? 'opacity-0' : 'opacity-100 animate-in fade-in'}`}>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: "200ms" }}>
                ...
                <CardDescription className="text-base md:text-lg max-w-xl mx-auto">
                  Deine Ziele sind gespeichert und dein Dashboard ist bereit. Lass uns dein Marketing auf das nächste Level bringen!
                </CardDescription>
              </div>
              <div className="flex flex-col sm:flex-row justify-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: "400ms" }}>
                <Button size="lg" variant="gradient" onClick={() => setLocation("/app/chats?prompt=Hey Houston! Ich bin gerade durch das Onboarding gekommen. Was sollte ich als erstes tun?")} className="text-lg px-8">
                  Mit Houston starten
                  <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
                <Button size="lg" variant="outline" onClick={() => setLocation("/app/dashboard")}>
                  Zum Dashboard
                </Button>
              </div>
            </div>
