...
export const CompetitorCard: React.FC<Props> = ({ competitors }) => {
  return (
    <Card title="Feindbild-Analyse" className="w-full max-w-lg mx-auto my-2">
      <div className="space-y-3">
        {competitors.length === 0 ? (
          <div className="text-center text-slate-500 py-4 italic border border-dashed border-white/10 rounded-lg">Keine Wettbewerber erkannt.</div>
        ) : (
            ...
            <div key={comp.id} className="bg-white/5 p-4 rounded-xl border border-white/5 hover:border-white/20 shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div className="bg-green-500/5 p-2 rounded border border-green-500/10">
                   <span className="text-green-400 font-bold block mb-1 uppercase text-[10px] tracking-wider">Stärken</span>
                ...
                <div className="bg-red-500/5 p-2 rounded border border-red-500/10">
                   <span className="text-red-400 font-bold block mb-1 uppercase text-[10px] tracking-wider">Schwächen</span>
